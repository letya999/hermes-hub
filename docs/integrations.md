...
| Telegram bot channel | Telegram Bot API through `communication-hub` | Channel adapter; sender allowlist and durable reply outbox |
...
| Atlassian | [official Rovo MCP](https://support.atlassian.com/atlassian-rovo-mcp-server/) | https://mcp.atlassian.com/v2/mcp + personal API token (Basic auth) |
...
Telegram has two independent paths: `communication-hub` is the Bot API channel adapter,
...
uses a personal API token supplied as `ATLASSIAN_EMAIL` and `ATLASSIAN_API_TOKEN`; the
runtime derives the Basic header without writing it to generated config. API-token auth
must be enabled by an Atlassian administrator and can expose fewer tools than OAuth;
some calls may require an explicit cloud ID. GitLab uses `glab`, not MCP. Provider
content never grants permission to create, edit, send, merge or trigger operations.
