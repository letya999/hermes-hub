...
| One-shot `hermes -z` process per job | **Replace** with a warm scale-to-zero Hermes `/api/sessions` and `/v1/runs` adapter | v0.2.1 published the compatibility fallback; v0.3.0 removes the one-shot executor after binary-identical real Docker acceptance | Roll back using released v0.2.1 artifacts after stop/drain/audit; current static mode uses native APIs and preserves home/spool | #14-#19 |
