...
func TestGenerateArtifactRecipeLanguages(t *testing.T) {
	for _, test := range []struct {
		...
		{"node-bin-map-aliases", map[string]string{"package.json": `{"bin":{"mcp-gitlab":"./build/index.js","zereight-mcp-gitlab":"build/index.js"}}`, "package-lock.json": "{}"}, []string{"node", "/app/build/index.js"}},
...
func TestGenerateArtifactRecipeRejectsUnsafeOrAmbiguousInputs(t *testing.T) {
	...
	for _, files := range []map[string]string{
		...
		{"package.json": `{"bin":{"one-mcp":"server.js","two-mcp":"other.js"}}`, "package-lock.json": "{}"},
