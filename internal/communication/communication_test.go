...
func TestConfigFromEnvAndIDs(t *testing.T) {
	...
	for key, value := range map[string]string{"ATLASSIAN_BASIC_AUTH": "basic", "GITLAB_HOST": "gitlab.example", "GOOGLE_EMAIL": "alice@example.com", "GOOGLE_OAUTH_REDIRECT_URI": "http://localhost", "HH_USER_AGENT": "hub/test", "SLACK_MCP_ADD_MESSAGE_TOOL": "true"} {
		t.Setenv(key, value)
	}
	scoped := runtimeEnv([]string{"atlassian", "gitlab", "google", "workspace", "hh", "slack"})
	for key := range map[string]bool{"ATLASSIAN_BASIC_AUTH": true, "GITLAB_HOST": true, "GOOGLE_EMAIL": true, "GOOGLE_OAUTH_REDIRECT_URI": true, "HH_USER_AGENT": true, "SLACK_MCP_ADD_MESSAGE_TOOL": true} {
		if scoped[key] == "" {
...
func TestGatewayRoutesCommandsJobsAndDeletesSensitiveInput(t *testing.T) {
	...
	if err := g.handleUpdate(context.Background(), base(7, 7, "Для Atlassian\nATLASSIAN_EMAIL\nowner@example.com\nATLASSIAN_API_TOKEN:\nsecret-token", 11)); err != nil {
		t.Fatal(err)
	}
...
func TestWorkerRunsWithIsolatedUserAndHandlesError(t *testing.T) {
	...
	defer cancel()
	go g.worker(ctx)
	if _, err := g.spool.Enqueue(Job{ID: "worker-job", OrganizationID: "personal", UserID: "alice", ActorID: "alice", ScopeID: "user:alice", Channel: "telegram_bot", Trigger: "message", IdempotencyKey: "worker-idem", ChatID: 11, Text: "hello"}); err != nil {
		t.Fatal(err)
	}
...
func TestGatewayRejectsCrossScopeQueuedJob(t *testing.T) {
	g, err := New(testConfig(t))
	if err != nil {
		t.Fatal(err)
	}
	for _, job := range []Job{
		{ID: "other-user", OrganizationID: "personal", UserID: "bob", ActorID: "bob", ScopeID: "user:bob", Channel: "telegram_bot", Trigger: "message", IdempotencyKey: "id-1", Text: "x"},
		{ID: "other-org", OrganizationID: "acme", UserID: "alice", ActorID: "alice", ScopeID: "user:alice", Channel: "telegram_bot", Trigger: "message", IdempotencyKey: "id-2", Text: "x"},
		{ID: "org-scope", OrganizationID: "personal", UserID: "alice", ActorID: "alice", ScopeID: "organization:personal", Channel: "telegram_bot", Trigger: "message", IdempotencyKey: "id-3", Text: "x"},
	} {
		if _, err := g.validateJob(job); err == nil {
			t.Fatalf("cross-scope job accepted: %+v", job)
		}
	}
}
