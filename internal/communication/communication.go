...
func ConfigFromEnv() (Config, error) {
	if path := os.Getenv("HUB_COMMUNICATION_CONFIG"); path != "" {
		...
		config.RuntimeURL = envOr("HUB_RUNTIME_URL", config.RuntimeURL)
		config.RuntimeToken = os.Getenv("HUB_RUNTIME_TOKEN")
	...
	return Config{OrganizationID: orgID, Users: []User{user}, TelegramToken: os.Getenv("TELEGRAM_BOT_TOKEN"), APIBaseURL: envOr("TELEGRAM_API_BASE_URL", "https://api.telegram.org"), SpoolDir: envOr("HUB_COMMUNICATION_SPOOL", "/state/gateway"), PollTimeout: 25 * time.Second, HermesCommand: envOr("HUB_HERMES_COMMAND", "hermes"), RuntimeURL: os.Getenv("HUB_RUNTIME_URL"), RuntimeToken: os.Getenv("HUB_RUNTIME_TOKEN")}, nil
}
...
func runtimeEnv(features []string) map[string]string {
	...
	if contains(features, "atlassian") {
		keys["ATLASSIAN_BASIC_AUTH"] = true
	}
...
func (s *Spool) Enqueue(job Job) (bool, error) {
	...
	if job.IdempotencyKey != "" {
		// ponytail: bounded directory scan is enough for the single-worker release;
		// replace with an indexed ledger only when queue size makes it measurable.
		for _, dir := range []string{"pending", "running", "done", "failed"} {
			...
			for _, entry := range entries {
				...
				var previous Job
				if json.Unmarshal(body, &previous) == nil && previous.IdempotencyKey == job.IdempotencyKey {
					return false, nil
				}
			}
		}
...
// HermesRunner is kept as a one-release source compatibility wrapper. Execution
// itself belongs to hermes-runtime.
...
func (r HermesRunner) Run(ctx context.Context, job Job, user User) (string, error) {
	...
	return (hubruntime.HermesRunner{Command: r.Command, Timeout: r.Timeout, Binding: hubruntime.Binding{
		UserID: userID, OrganizationID: job.OrganizationID, UserHome: filepath.Dir(user.StateDir),
		Features: user.Features, Env: user.Env,
	}}).Run(ctx, job)
}

type runtimeRunner struct{ client *hubruntime.HTTPClient }

func (r runtimeRunner) Run(ctx context.Context, job Job, _ User) (string, error) {
	return r.client.Run(ctx, job)
}

func processEnv(values map[string]string) []string { return hubruntime.ProcessEnv(values) }
...
func New(config Config) (*Gateway, error) {
	...
	if config.RuntimeURL != "" && config.RuntimeToken == "" {
		return nil, errors.New("runtime_token is required when runtime_url is configured")
	}
	...
	var runner Runner = HermesRunner{Command: config.HermesCommand}
	if config.RuntimeURL != "" {
		runner = runtimeRunner{client: hubruntime.NewHTTPClient(config.RuntimeURL, config.RuntimeToken)}
	}
	return &Gateway{config: config, users: users, spool: spool, api: newTelegramAPI(config.APIBaseURL, config.TelegramToken, config.PollTimeout+10*time.Second), runner: runner, now: time.Now}, nil
}
...
func (g *Gateway) worker(ctx context.Context) {
	...
	for {
		...
		if job, err := g.spool.ClaimJob(); err == nil && job != nil {
			g.busy.Store(true)
			user, validationErr := g.validateJob(*job)
			var response string
			var runErr error
			if validationErr != nil {
				runErr = validationErr
			} else {
				response, runErr = g.runner.Run(ctx, *job, user)
			}
			g.busy.Store(false)
			if runErr != nil {
...
func (g *Gateway) validateJob(job Job) (User, error) {
	user := g.user(job.UserID)
	if user.ID == "" || !user.Enabled || job.OrganizationID != g.config.OrganizationID || job.ActorID != job.UserID || job.ScopeID != "user:"+job.UserID || job.Channel == "" || job.Trigger == "" || job.IdempotencyKey == "" {
		return User{}, errors.New("job identity does not match communication binding")
	}
	return user, nil
}
